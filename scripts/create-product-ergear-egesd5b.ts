...
export async function createProductErgearEgesd5b(prisma: PrismaClient) {
  ...
  const brand = await prisma.brand.upsert({ where: { slug: 'ergear' }, update: { name: 'ErGear' }, create: { slug: 'ergear', name: 'ErGear' } });
  const product = await prisma.product.upsert({ where: { slug: PRODUCT_SLUG }, update: productData(category.id, brand.id), create: { ...productData(category.id, brand.id), slug: PRODUCT_SLUG } });
  const found = await prisma.productVariant.findFirst({ where: { product_id: product.id, sku: 'ergear-egesd5b-48x24-black' } });
  const variant = found ? await prisma.productVariant.update({ where: { id: found.id }, data: { size: '48x24', color: 'Black', material: null, is_active: true } }) : await prisma.productVariant.create({ data: { product_id: product.id, sku: 'ergear-egesd5b-48x24-black', size: '48x24', color: 'Black', is_active: true } });
  const prepared = [];
  const errors: string[] = [];
  for (const entry of entries(variant.id)) {
    const definition = await prisma.attributeDefinition.findUnique({ where: { key: entry.key } });
  ...
  for (const { entry, definition } of prepared) {
    ...
    const existing = await prisma.productAttribute.findFirst({ where });
    if (existing) await prisma.productAttribute.update({ where: { id: existing.id }, data }); else await prisma.productAttribute.create({ data });
  }
  return product;
}
