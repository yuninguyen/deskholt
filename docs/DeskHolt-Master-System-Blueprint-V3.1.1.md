...
- **Merchant Option Explosion confirmed with real numbers.** This single ASIN offers 8 sizes (32×19 in → 71×30 in) × 12 colors ≈ 96 merchant-side combinations, each a different price. This is a live instance of the exact scenario §12 warns about. **Decision: materialize only the one `ProductVariant` DeskHolt actually intends to track/promote** (here: 48×24 in, Black — the combination matching the featured affiliate price), not the full merchant option matrix. Revisit only if a second combination becomes independently worth tracking (own decision-relevant comparison, not merchant catalog completeness).
...
**Product #2 upgraded end-to-end, 2026-08-30.** `scripts/upgrade-product-shw-od92ak.ts` ran against the real database: category_id/brand_id/real UPC set, default variant updated (size/color), 13 sourced `VERIFIED` attributes persisted (including the new kg→lb-converted `max_load_lb` and cm→in-converted `max_height_in`), AffiliateLink price corrected to `$159.87`. Unexpectedly, this product already had `motor_count: 3` and `warranty_months: 12` (`UNVERIFIED`, no `source_url`) from a pre-existing row dated 2026-08-25 — predating this P2 dry run entirely, unrelated to today's work. Combined with the 13 new attributes, the product now has all 15 attributes with no manual admin step needed this time. `status`/`is_indexed` left untouched (`ACTIVE`/`false`, unchanged). This closes Product #2/10.

## Products #3–7 batch — 5 real standing desks, 2026-08-30

User supplied a 14-item Amazon batch. **Category scope check first:** only `standing-desks` has an Attribute Engine schema (35 `AttributeDefinition` rows via `prisma/seed-standing-desk-attributes.ts`) — `ergonomic-chairs`/`lighting`/`cable-management` have zero `AttributeDefinition`/`CategoryAttribute` rows. Per §8 (current checkpoint is Standing Desks only) and the same build-ahead-of-need discipline already applied to Merchant/Offer and unit conversion, **the 9 non-desk items (2 chairs, 5 lighting, 2 cable-management) are deferred** — no structured-attribute schema exists for their categories yet. Only the 5 standing desks proceed as Products #3–7. One ASIN in the deferred set (`B0BZ3GHM8N`, Litwaro cable tray) already exists in `prisma/seed.ts` — irrelevant while cable-management stays deferred.

**BLOCKER — two new real ontology decisions:**

- **FEZIBO (ASIN B0F8MHPVPH): `Top Material Type: FSC-Certified Wood`** — FSC is a sourcing certification, not a material composition; doesn't fit `desktop_material`'s enum (`MDF|BAMBOO|SOLID_WOOD|LAMINATE|ENGINEERED_WOOD`) without fabricating specificity the source doesn't give. **Decision: leave `desktop_material` unset for this product and set `Product.is_sustainable: true` instead** — using the field already on `Product` for exactly this kind of fact, rather than forcing a wrong material enum value.
- **OffiGo (ASIN B0FPFSYXNF): `Warranty Type: Lifetime`** — doesn't fit `warranty_months`'s `INTEGER` type (not a bounded duration). **Decision: leave `warranty_months` unset, mention "backed by a lifetime frame warranty" in the Product description text instead** of fabricating a large sentinel number (e.g. 999) that would misrepresent a real, specific manufacturer claim as an arbitrary numeric convention.

**NON-BLOCKER — new findings:**

- **OffiGo is genuinely `L_SHAPED`** — first real use of that `desktop_shape` enum member (previously only `RECTANGULAR` seen in Products #1–2). No schema gap; enum already supports it.
- **Contradictory source fields on Veken 55" (B0DWMJCQBX):** `Base: Carbon Steel` but `Frame Material Type: Engineered Wood` — almost certainly an Amazon listing template error (a wood top's frame can't itself be "Engineered Wood"). Decision: use the more specific `Base` field (`Carbon Steel` → `STEEL`) as source of truth, not the contradictory `Frame Material Type` field.
- **Claiks (B0BZ7GXM4M) has contradictory finish fields:** `Finish Type: Steel` vs `Furniture Finish: Black`, while `Top Material Type: Engineered Wood` — "Steel" as a desktop finish contradicts an engineered-wood top. Decision: leave `desktop_finish` unset for this product rather than store a self-contradictory value.
...
**Follow-up decision, 2026-08-30:** the one §59 criterion not met — "editor can enter data without developer intervention for normal cases" — traces to a concrete, real gap: there is no Admin UI for creating a Product's identity (name/category/brand/description/image/UPC). All 7 P2 products were created via one-off scripts, not through the Admin UI (only specification/attribute editing and lifecycle management have UI today). Decision: build a minimal "Create Product" Admin page/Server Action to close this gap — scoped to identity creation only (no AffiliateLink, no attributes/variants in the same flow; those stay on the existing specifications page and remain script-driven for now).

**Real bug found by first real manual use, 2026-08-31.** The user tried the new Create Product UI for real (the exact scenario the feature above was built to validate) and pasted a real Amazon product image URL into the Image URL field — legitimate, expected usage. `next.config.mjs` only whitelists `images.unsplash.com` (every prior product used the shared stock placeholder), so `next/image` crashed the entire `/category/standing-desks` page with an unconfigured-host error. Fixed by whitelisting `m.media-amazon.com` plus a `**.ssl-images-amazon.com` wildcard for Amazon's other CDN subdomains. This is exactly the kind of gap manual dogfooding surfaces that automated tests didn't — the whole test suite never exercised a non-Unsplash image host.

**Second real UX gap from the same first manual use, 2026-08-31.** The Create Product UI deliberately creates zero AffiliateLinks (by design, see above) — but `ProductCard.tsx` had never been exercised with `linkCount: 0` before (every one of the 27 prior products, 7 P2 + 20 legacy seed, always had at least one AffiliateLink). The homepage card rendered "Best price from / N/A" and "Compare 0 stores →", which reads as broken data rather than an expected "not priced yet" state. Fixed with a dedicated zero-offer copy path in the card component.

**Third real UX gap from the same first manual use, 2026-08-31.** The product detail page renders `product.description` in a plain `<p>` with no `white-space` handling, so a description pasted with embedded newlines (e.g. Amazon's bulleted "About this item" list, one bullet per line) collapses into one dense run-on paragraph — every prior product's description was a single line, so this was never exercised either. Plan: add `whitespace-pre-line` to that one `<p>` (`docs/superpowers/plans/2026-08-31-preserve-description-line-breaks.md`).
