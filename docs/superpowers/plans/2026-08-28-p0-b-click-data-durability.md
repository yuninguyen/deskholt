...
- [x] **Step 4:** Run `gitnexus_detect_changes()` before commit; confirm the diff touches only click-persistence/`/go`-route files, their tests, and `artifacts/p0-b/`/plan checkboxes — nothing from Spec 001 or unrelated work. Exposed CLI has no `detect_changes`; documented `status`/impact/context plus Git status/diff/search fallbacks were used, and no commit was made.
