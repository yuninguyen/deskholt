# DESKHOLT — MASTER SYSTEM BLUEPRINT & EXECUTION ROADMAP V3.1
...
## V3.1 reconciliation summary

V3.1 does **not** replace the Product Intelligence architecture. It changes execution authority by adding:

```text
V3 architecture
+
Production Safety Reconciliation
+
Execution Order Correction
```

Primary V3.1 corrections:

1. Admin auth fail-closed is P0.
2. Basic Index Gate moves to P0.
3. Structured Offer data must obey real price/availability/freshness.
4. Click persistence becomes P0 durable-event infrastructure.
5. Partial indexes must live in version-controlled Prisma migrations.
6. Seeds require explicit opt-in + database allowlist.
7. Merchant/Offer expansion moves behind P0.
8. Product #1–#10 moves behind P1 end-to-end record readiness.
9. Default Variant cannot be inferred from array order.
10. P0 completion requires implementation + automated verification + lint/typecheck/build.

---

# 0. MỤC ĐÍCH CỦA V3.1

V3.1 giữ nguyên Product Intelligence architecture của V3, đồng thời bổ sung **Production Safety Reconciliation + Execution Order Correction** từ code audit mới nhất. V3.1 là execution authority mới nhất và supersede thứ tự V1-alpha trước đây.
...
V3.1 supersedes các execution assumptions cũ sau:
...
Legacy redirect concept vẫn giữ, nhưng V3.1 nâng click durability thành P0.
...
↓
create click event with clickedAt = REQUEST TIME
↓
durable enqueue
↓
redirect
...
- click timestamp phải phản ánh **request time**, không phải worker processing time.
...
# 29. REDIS / CLICK QUEUE — P0 DURABILITY INVARIANT

Pattern destructive-pop trước persistence **không còn được chấp nhận**.

Không dùng:

```text
RPOP / destructive pop
↓
event biến mất khỏi queue
↓
DB insert
```

vì worker crash giữa pop và insert có thể làm mất click.
...
- queue có acknowledgement semantics hoặc processing-list equivalent;
- retry policy explicit;
- retry limit explicit;
- dead-letter path;
- worker command tồn tại;
- worker được wired vào PM2/process manager/deployment thật;
- worker health có thể kiểm chứng;
- queue lag có observability tối thiểu;
- event giữ `clickedAt` từ request time;
- integration test bao gồm success, crash/failure, retry và dead-letter.

Implementation có thể dùng Redis Streams + Consumer Groups hoặc một reliable processing-list pattern; V3.1 không ép một library cụ thể.
...
## 56.2. P0-B — EVENT + DATA DURABILITY

Definition of Done:

- click có `clickedAt` tại request time;
- queue claim → persist → ACK;
- retry + retry limit;
- dead-letter;
- worker command + process wiring + deployment verification;
...
queue success/failure/retry/dead-letter tests
...
# 66. CANONICAL V1 EXECUTION AUTHORITY — V3.1
...
### P0-B — Event + Data Durability

- clickedAt at request time;
- claim → persist → ACK queue semantics;
- retry / limit / dead-letter;
- deployed/wired worker;
