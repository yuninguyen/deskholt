...
export default async function ProductSpecificationsPage({
  ...
  searchParams: Promise<{ saved?: string; error?: string; count?: string; detail?: string }>;
}) {
  const { id } = await params;
  const { saved, error, count, detail } = await searchParams;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <ProductSpecificationsForm data={data} action={saveSpecificationsAction} />
    </div>
  );
}
